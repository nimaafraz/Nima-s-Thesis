...
% \textbf{
% In this chapter we will introduce the motivations for the research reported in this dissertation. review the motivations a
% }
...
Sharing the network reduces redundant \ac{CapEx} by splitting the infrastructure investment as well as the \ac{OpEx} through the economy of scale, for the operators. Therefore, lower \textit{"cost per bit"}, i.e., the delivery cost of a bit to a user over the network, can be achieved. As a result, lower \textit{cost per bit} can allow wider network deployments in under-served communities, and therefore, exponential growth in Internet penetration rate can be expected. This growth will bridge the digital divide bringing affordable access networks to the areas that would not have been served in conventional sole-ownership network deployment. Furthermore, with proper regulations in place, new network ownership/operation models can emerge that will impact competition by alleviating network entrance barriers. In other words, eliminating the prohibitive preliminary investment costs to enter the network as an operator facilitates innovation.
The advantages of network sharing are evident to the extent that in majority of countries some forms of network sharing have been mandated by the authorities \cite{nepal_sharing}. %Nonetheless, through fixed network sharing, these advantages can be enhanced by environmental advantages as it will require less infrastructure to be deployed.
...
In the multi-tenant \ac{PON} model, each \ac{VNO} will operate a virtual instance of their preferred \ac{DBA} that will generate a \ac{vBMap} for its share of the frame. Then the merging and final layout of the \ac{BMap} is facilitated by the merging engine. % while not imposing any additional scheduling delay. 
...
\newcommand{\RQa}{What are the motivations and implications for optical access network sharing?}
\newcommand{\RQb}{How to meet the technical requirements to enable fine-grained and dynamic optical access network sharing?}
\newcommand{\RQc}{Could monetization of excess resources incentivize the operators to share their excess resources with competitors?}
\newcommand{\RQd}{Could an economic robust double auction mechanism provide market participation incentives for inter-operator network sharing?}
...
% \toimprove{@Marco, Do you think it is better to introduce the Research Questions in the Introduction, background or the methodology chapter?}
\section{Research questions}
\toimprove{Only mention what you technically address in the paper}
\tochange{To integrate these with key contributions maybe}
\begin{itemize}
\item \RQa
\toimprove{You don't really address this in the background}
\item \RQb
\item \RQc
\item \RQd
\item \RQe
...
\end{itemize}
% \toimprove{Merge the two model and auction, produce more points, more focused (M) Trade reduction etc. (N)}
\subsection{Designing a Market Model and Auction Mechanism for Multi-Tenant \ac{PON}s}
% \tochange{Merge Market and Auction contributions else produce more points (M)}
\begin{itemize}
...
% \towrite{Where is the blockchain (M)}
\begin{itemize}
...
% challenge the assumption of the centrally trusted market holder, where we discussed how this could be unrealistic. We further provided an initial review of the potential solutions to this problem for future work.
...
The Structure of this dissertation is as follows:
    ...
    % \item In \autoref{cpt:methodology} we outline the research questions addressed in this dissertation and discuss the methods used to design experiments and evaluate the solutions proposed by the contributions of this dissertation. 
...
% \nobibliography{bib/dissertation.bib}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% In this section we list the dessimination undertaken throughut my Ph.D. 
\subsection{Peer-Reviewed}

 \begin{enumerate}
 
    \item \textbf{\underline{N. {Afraz}}}, F.~Slyne \emph{et~al.}, \say{Evolution of Access Network Sharing and Its Role in 5G Networks}, \emph{Applied Sciences}, vol.~9, no.~21, p. 4566, Oct 2019.
    
    \item A.~{Elrasad}, \textbf{\underline{N. {Afraz}}}, M.~{Ruffini} , \say{Virtual Dynamic Bandwidth Allocation Enabling True PON Multi-Tenancy}, in \emph{2017 Optical Fiber Communications Conference and Exhibition (OFC)}, March 2017, pp. 1--3.
        
    \item \textbf{\underline{N. {Afraz}}}, F.~Slyne, M.~Ruffini, \say{Full PON Virtulisation Supporting Multi-Tenancy Beyond 5G [Invited]}, in \emph{OSA Advanced Photonics Congress (AP) 2019 (IPR, Networks, NOMA, SPPCom, PVLED)}.\hskip 1em plus 0.5em minus 0.4em\relax Optical Society of America, 2019, p. NeT2D.2.
    
    \item M. Ruffini, A. Ahmad, S. Zeb, \textbf{\underline{N. {Afraz}}}, and F. Slyne, \say{The Virtual DBA: Virtualizing Passive Optical Networks to Enable Multi-Service Operation in True Multi-Tenant Environments}, \emph{Journal of Optical Communications and Networking (JOCN)}, Jan. 2020.
    

    \item \textbf{\underline{N. {Afraz}}} and M.~{Ruffini}, \say{A Sharing Platform for Multi-Tenant PONs}, \emph{Journal of Lightwave Technology (JLT)}, vol.~36, no.~23, pp. 5413--5423, Dec 2018.

    \item \textbf{\underline{N. {Afraz}}} and M.~{Ruffini}, \say{A Marketplace for Real-Time Virtual PON Sharing}, in \emph{2018 Asia Communications and Photonics Conference (ACP)}, Oct 2018, pp. 1--3.

    \item \textbf{\underline{N. {Afraz}}}, A.~{Elrasad}, M.~{Ruffini}, \say{DBA Capacity Auctions to Enhance Resource Sharing Across Virtual Network Operators in Multi-Tenant PONs}, in \emph{2018 Optical Fiber Communications Conference and Exposition (OFC)}, March 2018.
   
    \item \textbf{\underline{N. {Afraz}}}, A.~{Elrasad}, H.~Ahmadi,  M.~{Ruffini}, \say{Inter-Operator Dynamic Capacity Sharing for Multi-Tenant Virtualized PON}, in \emph{2017 IEEE 28th Annual International Symposium on Personal, Indoor, and Mobile Radio Communications (PIMRC)}, Oct 2017, pp. 1--6.

    
    \item  \textbf{\underline{N. {Afraz}}} and M.~{Ruffini}, \say{A Distributed Bilateral Resource Market Mechanism for Future Telecommunications Networks}, in \emph{2019 IEEE Global Communications Conference (GLOBECOM)}, December 2019.
    
    \item \textbf{\underline{N. {Afraz}}}. and M.~Ruffini, \say{{5G} network slice brokering: A Distributed Blockchain-based Market}, in \emph{2020 European Conference on Networks and Communications (EuCNC): Network Softwarisation (NET) (EuCNC2020 - NET)}, Dubrovnik, Croatia, Jun. 2020 [under review].
    
    \item {A. {Khatoon}}, \textbf{\underline{N. {Afraz}}}, \say{Approaches to Adopt Blockchain Technology for the Internet of Things}, in IEEE Technology and Society Magazine [under review].
    
    \item {M.~Hajizadeh}, \textbf{\underline{N. {Afraz}}}, M.~Ruffini, and T.~Bauschert, \say{Collaborative Cyber Attack Defense in {SDN} Networks using Blockchain Technology}, in \emph{2nd International Workshop on Cyber-Security Threats, Trust and Privacy management in Software-defined and Virtualized Infrastructures (SecSoft 2020)}, Ghent, Belgium, Jul. 2020 [under review].
...
\subsection{Patent}
    \begin{enumerate}
    \item M.~Ruffini, A.~Elrasad, \textbf{\underline{N. {Afraz}}}, \say{System and {Method} for {Dynamic} {Bandwidth} {Assignment} (DBA) {Virtualization} in a {Multi}-{Tenant} {Passive} {Optical} {Network}}, Patent WO/2018/167\,318, Sep., 2018.
    %   \item \bibentry{Nima-vDBA-patent}
    \end{enumerate}
    
\subsection{Tutorial Talks}
\begin{enumerate}
    \item H.~Ahmadi, I.~Macaluso, M.~{Ruffini}, \textbf{\underline{N. {Afraz}}}, \say{Blockchain Technology and Smart Contracts in 5G and Beyond Networks [Delivered]}, 2019, European Conferences on Networks and Communications (EUCNC).
    
    \item H.~Ahmadi, I.~Macaluso, M.~{Ruffini}, \textbf{\underline{N. {Afraz}}}, \say{Blockchain Technology and Smart Contracts in 5G and Beyond Networks [Accepted and to be Delivered]}, 2020, IEEE International Conference on Communications (ICC).\end{enumerate}
...
\begin{itemize}
    \item \textbf{\underline{N. {Afraz}}}, V.~Chaudhary \emph{et~al.}, \say{Optimizing Wholesale Intercarrier Settlement with Hyperledger Fabric Blockchain}, \emph{Solution Brief}.\hskip 1em plus 0.5em minus 0.4em\relax Telecom Special Interest Group, 2019.
\end{itemize}
% \section{Research Grant Proposals}
% The following research proposals have been submitted to funding agencies to pursue the future works of this dissertation as a post-doctorate researcher:
% \begin{enumerate}
%     \item The Government of Ireland Postdoctoral Fellowship Programme (1-Year): The project aims to provide a robust evaluation methodology for blockchain use cases in the communications industry. Furthermore, it plans to extend the work on distributed markets framework for network infrastructure sharing.
    

%     \item I have contributed to putting forward a proposal for the Marie Skłodowska-Curie Actions Innovative Training Networks 2020. More than ten academic/industry partners are collaborating on this proposal. If successful, I will be a co-supervisor for an early stage researcher (Ph.D. student) on a blockchain-oriented topic. 

% \end{enumerate}
% \section{Public and Community Engagement}
...
% % \begin{figure}[h]
% % \centering
% % \fbox{\includegraphics[width=0.8\linewidth]{Figures/ty.png}}
% % \label{ty}
% % \end{figure}
% \begin{enumerate}
%       \item Hosting and supervising transition year students from Larkin Community College in Dublin to work on a project entitled ``Own your own Skype – WebRTC setup and test``. The students became familiar with the environment of an academic research center and had hands-on experience with building a communication network.
%     % https://connectcentre.ie/news/ty-immersion-week/


%     \item Delivered a Guest Lecture on ``Blockchain Beyond Coins, Next Generation Networks``, School of Computer Science and Statistics at Trinity College Dublin. 
% \end{enumerate}
