...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \section{Distributed Market Model}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%SECTION%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% \textcolor{red}{[Need reference]}. 

%{Marco: I don't really get this part. Before privatisation there was only one public operator, so privatisation did enable, slowly, a level of competition}.
%This goes along with the trend of privatizing the communications infrastructure both in developed and developing countries where the public authority's control over the infrastructure management has shrank to some high-level regulatory efforts. Regardless of the benefits brought to the telecomms sector by privatization (e.g., infrastructure investment growth) it has also made the market entrance more sophisticated for smaller operators. A Virtual Network Operator (VNO) for instance does not have a negotiation advantage as it solely relies on the private infrastructure provider for acquiring the necessary resources to reach their customers. Therefore, the power dynamics in these scenarios known as infrastructure sharing is experiencing considerable shifts and if not addressed adequately could become a bottleneck for innovative use of the infrastructure.


% \textcolor{red}{Make tyhe analogy with bitcoin. The same way bitcoin used blockchain to remove any central bank authority, we now use make use of blcochain through Hyperledger Fabri to eliminate the need for a trusted third party, which has been knowingly difficult in incumbent-driven telecomms markets (apologies for my english.. just writing very quickly)}

% \textcolor{red}{Also, most people don;t have a clue in what's new in blockchain, so make sure to say something like.. this is the first time that ... also make a hint to it in the abstract.}


% \textcolor{red}{I think this section can be merged with the introduction. Then you can have a section on explaining the use case,  why it's needed and how is it modelled with blockchain.}
...
% physical infrastructure is owned by an incumbent operator, which also offers services on the network. The main concern about current private/sole ownership (see \figureautorefname~\ref{Fig_access}) and sharing scheme is the situation where the current Optical Infrastructure Provider (OInP) is a private entity who is also utilizing the same infrastructure to serve its customers. 
...
% \textcolor{red}{I would call the network control in the figure just "Network"}

%and a new verification  to satisfy their In this case, the Over-The-TOP  (OTT-VNOs) have no trust incentive towards the OInP to operate the market (i.e., the resource redistribution mechanism) in a centralized manner. 
%The conventional ownership models typically rely on a central trusted authority to invest in deployment, oversee and regulate the operation and provide revenue assurance.  
%In other words, a regulated monopoly (perhaps state-owned) is no longer the optimal market structure .



% \textcolor{red}{Not sure about the figure. 1) The incumbent is always private; 2) every VNO in principle offers multiple service, so I would eliminate the service type from the boxes; 3) I would simplify it maybe with just network and services and only have the open access model and the typical incumbent-driven model (trust-less)}.
\begin{figure}[htbp]
%\vspace{-3mm}
  \centering
...
% %\vspace{-3mm}
%   \centering
%   \includegraphics[width=0.99\textwidth]{ownership.pdf}
% %\vspace{-4mm}
% \caption{Access Infrastructure Sharing Models}
% \label{Fig_access}
% %\vspace{-5mm}
...
% \textcolor{red}{[need strong reference]}.
...
%new trust-less market environment where the traditional role of the mediator is to be replaced with a mechanism that does not rely on a third-party to provide trust. 
%However, the market mechanisms introduced in the literature in multi-tenant \ac{PON} infrastructure \cite{8488596} and similar communications infrastructure sharing problems such as spectrum sharing \cite{7194843} and cloud/data-center resources by design rely entirely on a central authority to conduct the market mechanism and make the final decision making process. For instance in \cite{8488596} where multiple VNOs are cohabiting the same\ac{PON}infrastructure, the OInP is relied/trusted upon to conduct periodical\ac{DBA}auctions which enables reallocating the unused capacity of the under-loaded VNOs. However, this does not account for the case where the OInP is a VNO at the same time thus cannot be blindly trusted to conduct the auction. Therefore, we propose enhancing the market model with a verification layer that uses blockchain to keep immutable records of the transaction, run the auction rounds using a smart contract and then automatically penalize any manipulation through triggering penalty transactions pre-defined in the smart contract.

% \textcolor{red}{Here you are a bit general. Make the concrete examples of multi-tenant PONs, where now that VNOs can schedule their own capacity they have no incentive in leaving unallocated capacity in any frame as this will go to their competitors}
%Blockchain will provide the lacking trust through 
\begin{figure*}[htbp]
%\vspace{-3mm}
  \centering
...
The solution we propose for decentralizing the market is to adopt blockchain as a solution to provide:
% 1- Reliable and robust transaction flow through Blockchain consensus mechanisms such as RAFT \cite{Ongaro:raft}, 
%     2- Transparent transactions and record-keeping enabled by the distributed ledger technology, and 
%     3- Immutable transaction logic enabled by the smart contract technology: one party cannot unilaterally alter the terms of the contract.
\begin{enumerate}
...
% \textcolor{red}{Try to match the description you give above to Figure 3., also adding some more details if needed}



% GPON allows very fine-grained transmission scheduling (125 microseconds) however typical killer applications of \ac{PON} do not necessarily require such frequent scheduling (e.g. fronthaul over \ac{PON} where variable CPRI rates would only vary on an hourly/daily/monthly... basis). Thus, we make use of the Scheduling interval feature of the GPON networks which allows scheduling over multiples of frames.
...
%\vspace{-5mm}
\end{figure*}
\subsubsection{Experiments and Results} \label{sec:results}
% \columnwidth\textcolor{red}{If you have a section before on the model, then this is OK. Put emphasis on the fact that this is one of the first work where blockchain transactions occur across multiple machines, rather than just simulated in one}
...
\section{ICC Paper: 5G Network Slicing}
...
% Auction mechanism have been widely used to address a wide range of issues related to 5G network \cite{8306104} including resource allocation.




% A wireless infrastructure business ecosystem consists of the following players:

% \begin{enumerate}
%     \item Infrastructure Providers (InPs):
%     \item MVNOs:
% \end{enumerate}

In the rest of this section, we will elaborate more on the definition of a network slice as a commodity and provide a short review on its common market mechanisms. Furthermore, we discuss the need for quality assurance and the tools for enforcing the terms of the trade. 
\begin{figure}[htbp]
            % \vspace{-4mm}
    \centering
...
% Majority of the slice brokering platforms rely on the economic robustness of their designed mechanisms. Therefore the aspect of 



% \begin{figure*}[htbp]
%     \centering
%     \includegraphics[width=0.89\textwidth]{figs-copy/layers.png}
%     \caption{The overall Schematic}
%     \label{fig:flow}
% \end{figure*}
...
%     \includegraphics[width=0.9\linewidth]{figs-copy/cpu.pdf}
%     \caption{}
% \end{subfigure}   
% \begin{subfigure}[t]{0.5\hsize}
%     \includegraphics[width=0.9\linewidth]{figs-copy/mem.pdf}
%     \caption{}
% \end{subfigure}
...
% \begin{subfigure}[t]{0.5\hsize}
%     \includegraphics[width=0.9\linewidth]{figs-copy/net.pdf}
%     \caption{}
% \end{subfigure}
...
% In this paper we  a practical implementation of a market mechanism to address the slicing problem in 5G networks. However, the proposed platform could be used for other smart contracts with different logic covering interference etc.
